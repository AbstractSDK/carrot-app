...
pub fn deposit_preview(
    ...
    let target_strategy = load_strategy(deps)?;
    let DepositStrategy {
        withdraw_strategy,
        deposit_msgs,
...
pub fn withdraw_preview(
    ...
    let funds = strategy.withdraw_preview(deps, withdraw_share, app)?;

    let msgs = strategy.withdraw(deps, withdraw_share, app)?;
...
pub fn update_strategy_preview(
    ...
    let old_strategy = load_strategy(deps)?;
